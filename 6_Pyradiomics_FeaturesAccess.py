"""
影像组学批量特征提取代码
查阅组学特征的意义，详见：https://blog.csdn.net/weixin_46428351/article/details/123592586
代码中采用的yaml文件，这是我自己修改后用于提取MRI的特征的文件，见主文件夹中的2_MRI_1210_features.yaml文件
提取到的特征在去除版本信息特征后的数量为1210个
"""
...
def radiomics_feature_access(all_path):
    ...
    paramPath = './2_MRI_1210_features.yaml' 
    ...
    features_df = pd.read_excel("./radiomics_features_CT.xlsx") 
    features_df.insert(loc=0, column='Name', value=img_name_list) #在特征表中第一列位置插入图像名字列
    num2 = 0
    unuseful_features = []
    for feature in features_df.columns:
        if 'diagnostics' in feature:       # 所有带有diagnostics字样的放射组学特征均为软件版本信息，为无用特征，可以放心去除！
    ...
    features_df.to_excel("./radiomics_features_CT.xlsx",index=False)
radiomics_feature_access('./IMG_Seg')
